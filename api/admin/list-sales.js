...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (error) return res.status(500).json({ error:'DB error (sales)', detail:error.message });
    return res.status(200).json({ items: data || [], nextOffset: offset + (data?.length||0) });
  } catch (e) {
    return res.status(500).json({ error:'Server error (sales)', detail:String(e?.message||e) });
  }
}
