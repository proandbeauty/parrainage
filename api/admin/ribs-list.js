...
export default async function handler(req, res) {
  ...
  try {
    ...
    // NOTE: la relation "referrer:referrers(...)" suppose une FK configurée.
    // Si ta relation a un nom explicite, utilise par ex.:
    // 'referrer:referrers!bank_accounts_referrer_id_fkey ( first_name, last_name, email, referral_code )'
    let q = supabase
      .from('bank_accounts')
      .select(`
      ...
      .limit(1000); // on récupère large puis on filtre côté Node (robuste avec la recherche locale)
    ...
    if (String(req.query.format || '').toLowerCase() === 'csv') {
      ...
      for (const r of rows) {
        const ref = r.referrer || {};
        lines.push(
          [r.id, r.status, r.created_at || '', r.holder_name || '', r.iban || '', r.bic || '', ref.email || '', ref.referral_code || '']
